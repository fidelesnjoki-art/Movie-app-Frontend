...
function Profile() {
  ...
  const myPosts = useSelector((s) =>
    s.posts.items.filter((p) => p.user.name === user?.name || p.user.name === user?.email)
  );
          ...
          <section>
            {myPosts.length === 0 ? (
              <div className="text-center py-16 bg-white/3 border border-white/5 rounded-2xl text-gray-500">
                ...
                <Link to="/posts/create" className="mt-3 inline-block text-sm text-[#f6b042] hover:underline">Write your first review</Link>
              </div>
            ) : (
          ...
          <section>
            {watchlist.length === 0 ? (
              <div className="text-center py-16 bg-white/3 border border-white/5 rounded-2xl text-gray-500">
                ...
                <Link to="/discover" className="mt-3 inline-block text-sm text-[#f6b042] hover:underline">Browse Discover</Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {watchlist.map((movie) => (
                  <div key={movie.id} className="flex items-center gap-3 bg-white/3 border border-white/5 rounded-xl p-3 hover:bg-white/5 transition-colors">
                    <div className="w-10 h-14 rounded-md bg-white/5 overflow-hidden shrink-0">
                      {movie.poster_path ? (
                        <img src={${IMG_THUMB}${movie.poster_path}} alt={movie.title} className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-gray-600 text-xs">?</div>
                      )}
                    </div>
                    <Link to={/movies/${movie.id}} className="flex-1 text-sm text-gray-200 truncate hover:text-white transition-colors">
                      {movie.title}
                    </Link>
                    <button onClick={() => dispatch(toggleWatchlist(movie))} className="text-xs text-gray-500 hover:text-red-400 transition-colors shrink-0">Remove</button>
                  </div>
                ))}
              </div>
            )}
        ...
        {activeTab === "clubs" && (
          <section>
            {joinedClubs.length === 0 ? (
              <div className="text-center py-16 bg-white/3 border border-white/5 rounded-2xl text-gray-500">
                <p className="text-sm">You haven't joined any clubs.</p>
                <Link to="/clubs" className="mt-3 inline-block text-sm text-[#f6b042] hover:underline">Browse clubs</Link>
              </div>
            ) : (
                ...
                {joinedClubs.map((club) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-gray-500 mt-0.5">{club.members} members · {club.genre}</div>
                    </div>
                  </Link>
                ))}
              </div>
        ...
        {activeTab === "following" && (
          <section>
            {followingUsers.length === 0 ? (
              ...
              <div className="space-y-3">
                {followingUsers.map((u) => <ProfileCard key={u.id} user={u} />)}
              </div>
            )}
          </section>
        )}
